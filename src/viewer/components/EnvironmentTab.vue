    ...
    <template v-else>
      <div class="env-cards">
        <div v-for="(card, idx) in cards" :key="idx" class="env-card">
          ...
          <!-- Comparison Header -->
          <div v-if="!card.noCompare" class="compare-header">
            <span class="compare-header-spacer"></span>
            <div class="compare-header-cols">
              <span>Recorded</span>
          ...
          <div class="env-card-body">
            <div v-for="(item, i) in card.items" :key="i" class="env-row">
              ...
              <div class="env-values" v-if="!card.noCompare">
...
interface EnvCard { title: string; items: EnvItem[]; noCompare?: boolean; }
...
const cards = computed(() => {
  ...
  const infoItems: EnvItem[] = [];
  if (env.generatedAt) {
    infoItems.push({ key: 'Created At', value: formatDate(env.generatedAt), currentValue: 'N/A', matches: false });
  }
  if (env.url) {
    infoItems.push({ key: 'URL', value: env.url, currentValue: 'N/A', matches: false });
  }
  if (env.timezone) {
    infoItems.push({ key: 'Timezone', value: env.timezone, currentValue: 'N/A', matches: false });
  }

  const infoCard: EnvCard[] = infoItems.length > 0 ? [
    { title: '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"></circle><polyline points="12 6 12 12 16 14"></polyline></svg> Info', items: infoItems, noCompare: true },
  ] : [];
...
.compare-header { display: flex; padding: 6px 16px; font-size: 10px; text-transform: uppercase; color: var(--text-secondary); border-bottom: 1px solid var(--border-color); background: var(--bg-header); }
.compare-header-spacer { min-width: 120px; flex-shrink: 0; }
.compare-header-cols { display: flex; gap: 10px; flex: 1; }
.compare-header-cols span { width: 48%; }
